...
namespace DodiDownloader.Ui
{
    public class MirrorWindow
    {
        ...
        public MirrorWindow(DodiCache dodiCache, string gameName)
        {
        ...
        public async Task Run(bool regenerate)
        {
            ...
            try
            {
                IEnumerable<Mirror> mirrors = await _dodiCache.TryGetMirrors(_gameName, regenerate);
                ...
                Label description = new Label(_dodiCache.GetDescription(_gameName) ?? "")
                {
                    X = 1,
                    Y = 0,
                    Width  = Dim.Fill(1),
        ...
        private void OnListViewItem(ListViewItemEventArgs args)
        {
            ...
            IEnumerable<Mirror> mirrors = _dodiCache.TryGetMirrors(_gameName, false).Result;
